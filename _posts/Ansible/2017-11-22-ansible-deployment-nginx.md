...
- name: Uncompression Ruby Software
  unarchive: 
    src: /tmp/\{\{ ruby_ver \}\}.tar.gz
    dest: /tmp/
    remote_src: yes
- name: Setting Ruby Configure
  shell: ./configure
  args:
    chdir: /tmp/\{\{ ruby_ver \}\}
- name: Compile Ruby
...
- name: Delete Ruby compression files
  file: 
    path: /tmp/\{\{ ruby_ver \}\}.tar.gz
    state: absent
- name: Delete Ruby Decompression directory
  file: 
    path: /tmp/\{\{ ruby_ver \}\}
    state: absent
```
